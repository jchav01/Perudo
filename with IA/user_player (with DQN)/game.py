...
class CustomGameEnv:
    def __init__(self, num_players, DQN_agent_wrapper, user = False):
        self.num_players = num_players
        self.user = user
        self.players = self.create_players(num_players, DQN_agent_wrapper, user)
    ...
    def reset(self):
        ...
        self.players = self.create_players(self.num_players, self.DQN_agent, self.user)
    ...
    def create_players(self, num_players, DQN_agent_wrapper, user):
        players = []
        ...
        if user:
           players.append(Player("User", strategy='user'))
    ...
    def show_user_dice(self):
        for p in self.players:
            if p.strategy == 'user':
                print(f"{p.name}: {p.get_dice()} {p.organize_dice()}")
